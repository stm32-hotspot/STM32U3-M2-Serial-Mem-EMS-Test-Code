...
typedef struct {
  ...
  uint8_t SubAdrBytes[4]; // first byte is sub address, if any // up to 4 byte subaddress can be stored here. When not needed, could be reused as small data buffer.
...
int32_t I2C_MasterIO_ConfigTimings( I2C_MasterIO_t* pM, uint32_t MinBps, uint32_t MaxBps ); // here should be zero
