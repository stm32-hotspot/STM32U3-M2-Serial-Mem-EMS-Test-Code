...
SerialMemGlobalInfo_t SerialMemGlobalInfo =
{
  ...
  .MaxFreq_MHz = { 200, 0, 0, 0 },       // 
  .Vmin_V_x100 = 162,
  .Vmid_V_x100 = 0,   // support 2 voltage ranges with different speed support
  .Vmax_V_x100 = 198,
  
  
  .SizekByte = 32768, // 32768 = 256 Mbit -1 not available
  .BlockSizeByte = 1024*2048L,   // -1 not available
  .SectorSizeByte = 2048,  // -1 not available
  .PageSizeByte = 2048,    // -1 not available
...
void MX_NOR_ReadCR2(uint8_t index) {
  ...
  TT = DO_READ | NO_ALT;
  SM_PresetCommandParameters(&gCommand, TT, 0x4040, CR2_Adresses[index], 2, 4);
...
void MX_NOR_WriteCR2(uint8_t index, uint8_t byteval) {
  ...
  TT = DO_WRITE | NO_ALT;
  SM_PresetCommandParameters(&gCommand, TT, 0xC0C0, CR2_Adresses[index], 1, 0); // WRITE CR2
  SM_DoTransaction(&hospi1, TT, Mem_CR2Dual); 
}
...
{
  // prepare autopoll
  // do not use DQS for now DQS_Enabled = 1; // DQS is always on
