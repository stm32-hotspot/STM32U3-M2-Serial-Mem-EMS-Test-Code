...
#include "main.h"
#include "SerialMem.h"
// MX35LF4GE4 Serial Quad NAND Macronix memory
...
void MX_NOR_WriteEnable(void) { // OK
...
void MX_NOR_WriteDisable(void) { // OK
  switch(MemoryConfigurationMode) {
...
uint8_t MX_NOR_ReadStatusRegister(void) { // OK
...
//      SM_DoTransaction(&hospi1, TT, Mem_CR);
      ...
      TT = DO_READ | NO_ALT | NO_ADR;
      SM_PresetCommandParameters(&gCommand, TT, 0x9F, 0, 3, 1);
...
void MX_NOR_ReadCR2(uint8_t index) { // OK Get features
  
  switch(MemoryConfigurationMode) {
    
  case 0: // 1bit STR
    TT = DO_READ | NO_ALT | ADR_8BIT_ONLY;
    SM_PresetCommandParameters(&gCommand, TT, 0x0F, CR2_Adresses[index], 1, 0);
...
void MX_NOR_WriteCR2(uint8_t index, uint8_t byteval) { // OK Set features
  ...
  case 0: // 1bit STR
    MX_NOR_WriteEnable();
    TT = DO_WRITE | NO_ALT | ADR_8BIT_ONLY;
    SM_PresetCommandParameters(&gCommand, TT, 0x1F, CR2_Adresses[index], 1, 0); // WRITE CR2
    SM_DoTransaction(&hospi1, TT, Mem_CR2Dual); 
    break;
