...
#include "main.h"
#include "SerialMem.h"
#include "M2SM_EM128LX.h"
// SerialMemory Description Structure
// Phase 1: Adapt Macronix APIs for Everspin MRAM
...
SerialMemGlobalInfo_t SerialMemGlobalInfo =
{
  ...
  .CPN = "EM128LX",
  .ManufacturerName = "EVERSPIN",
  .MemoryType = MRAM_Type, // SFDP, Flash, Sector erase, Page write model
  .JEDEC_ID = {0x6B, 0xBB, 0x18 }, // EVER 1.8V 128MB
  .Support_Features = {         
                      .SFDP_Supported = 1,
  ...
  .SizekByte = 163842, // -1 not available 128 MBit (it is a dual dice, we support single dice now)
  .BlockSizeByte = 1024*32L,   // -1 not available
...
uint8_t MX_NOR_ReadStatusRegister(M2SerMem_t * pM2SM) {
  ...
  pM2SM->Registers[EM_STATUS_REG_INDEX] = Bytes[0];
...
void MX_NOR_WriteEnable(M2SerMem_t * pM2SM) {
  ...
  MX_NOR_ReadStatusRegister(pM2SM);// bonus for debug
}
...
void MX_NOR_DFIM_Once(M2SerMem_t * pM2SM) {
  ...
  uint8_t Bytes[2] = { 0x6B, 0 };
  
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    {   0x81, 0x0000001E, 3, 1, 0 }, // 1bit STR
    { 0x04FB, 0x00000000, 0, 0, 0 }, // 8bit STR
    { 0x04FB, 0x00000000, 0, 0, 0 }  // 8bit DTR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  ...
  SM_PresetCommandParameters(pM2SM, &ST);// 
  SM_DoTransactionWrite(pM2SM, Bytes); // enable DFIM

  Bytes[0] = 0;
  HAL_Delay(1000);
  
  SM_DoTransactionWrite(pM2SM, Bytes); // disable DFIM
...
void MX_NOR_ReadCR2(M2SerMem_t * pM2SM, uint8_t index) {
  ...
  pM2SM->Registers[EM_CR_0000 + index] = Bytes[0];
}


void MX_NOR_WriteCR2(M2SerMem_t * pM2SM, uint8_t index, uint8_t byteval) {
...
void MX_NOR_ReadAllRegisters(M2SerMem_t * pM2SM) {
  ...
  MX_NOR_ReadStatusRegister(pM2SM);
  //-MX_NOR_ReadCR(pM2SM);
  MX_NOR_ReadAllCR2(pM2SM);
  //-MX_NOR_ReadSecurityRegister(pM2SM);
}
...
void MX_NOR_ReadData(M2SerMem_t * pM2SM, uint32_t Addr, uint8_t* pBytes, uint32_t ByteSize) {
  ...
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    {   0x0C, 0x00000000, 4, 0, -1 }, // 1bit STR
...
void MX_NOR_ProgramData(M2SerMem_t * pM2SM, uint32_t Addr, uint32_t SizeByte) {
  ...
  // Not needed, use Write command
}


void MX_NOR_WriteData(M2SerMem_t * pM2SM, uint32_t Addr, uint32_t SizeByte) {

  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x12, 0x00000000, 4, 0, 0 }, // 1bit STR
    { 0x8E, 0x00000000, 4, 0, 0 }, // 8bit STR
    { 0x8E, 0x00000000, 4, 0, 0 }  // 8bit DTR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs)) while(1);
  SerialTransaction_t ST = STs[mode];
...
//  SerialConfiguration_t t;
  ...
  case 0: // 1bit STR
      //-MX_NOR_ReadCR(pM2SM);
      MX_NOR_ReadJedec(pM2SM, JedecInfo);
      MX_NOR_ReadSFDP(pM2SM, SFDP);
      MX_NOR_ReadData(pM2SM, 0x00000000, DataSegment, 4096);
      MX_NOR_ReadAllCR2(pM2SM);
      //-MX_NOR_ReadSecurityRegister(pM2SM);
    break;
...
//========================================================
int8_t MX_NOR_GetImpedence(M2SerMem_t * pM2SM) { // to be tested

  //-MX_NOR_ReadCR(pM2SM);
  return pM2SM->Registers[EM_CR_0000] & 0x07; // hard coded for now
}

void MX_NOR_SetImpedence(M2SerMem_t * pM2SM, int8_t imp) { // to be tested
  
  uint8_t Bytes[2];
  //-MX_NOR_ReadCR(pM2SM);
  Bytes[0] = pM2SM->Registers[EM_CR_0000] & ~0x07;
  Bytes[0] |= imp & 0x07;
  //- MX_NOR_WriteCR(pM2SM, Bytes[0]);
}

int32_t MX_NOR_GetDummyCycles(M2SerMem_t * pM2SM) { // untested
  
//  uint8_t Bytes[2];
  MX_NOR_ReadCR2(pM2SM, 5);
  return pM2SM->Registers[EM_CR_0000] & ~0x07;
}

void MX_NOR_SetDummyCycles(M2SerMem_t * pM2SM, int32_t dummycycles) { // untested
  
  uint8_t Bytes[2];
  MX_NOR_ReadCR2(pM2SM, 5);
  Bytes[0] = pM2SM->Registers[EM_CR_0000] & ~0x07;
  Bytes[0] |= dummycycles & 0x07;
  MX_NOR_WriteCR2(pM2SM, 5, Bytes[0]);
}
...
MX25LM5145G_Test(void)
{
  // prepare autopoll
  SM_ConfigInit(pM2SM);
  pM2SM->Config.MatchMask  = 0x01; // WE is not cleared by the MRAM
...
//    NOPs(2);
  ...
  //MX_NOR_WriteCR(pM2SM, Impedance);
//  HAL_XSPI_SetClockPrescaler(&hxspi1, 1);
  ...
  case 1:
    ...
    MX_NOR_WriteEnable(pM2SM);
    MX_NOR_ReadStatusRegister(pM2SM);
    MX_NOR_WriteDisable(pM2SM);
    MX_NOR_ReadStatusRegister(pM2SM);
    
    //while(1); // stop here for now
    
    //MX_NOR_EraseSegment(pM2SM, 0);
    
    DataBufferFill(0,512,0x55,1);
    MX_NOR_ReadInfo(pM2SM);
    DataBufferFill(0,1024,0,1);
    MX_NOR_WriteData(pM2SM, 0x00000000, 256);
    DataBufferFill(0,1024,0,0);
    MX_NOR_ReadInfo(pM2SM);
    DataBufferFill(0,1024,0x80,0xFF);
    MX_NOR_WriteData(pM2SM, 0x00000100, 16);
    MX_NOR_ReadInfo(pM2SM);
    break;
  case 2: // test going to 8 bit SDR mode then back to 1 bit SDR mode
    // going to 8 bit SDR mode
    MX_NOR_WriteCR2(pM2SM, 0,0x01); // switch to 8 bit SDR mode
    MX_NOR_SetMode(pM2SM, 1);

    // do something
    MX_NOR_ReadInfo(pM2SM);  
    //MX_NOR_EraseSegment(pM2SM, 0x00010000); // second 64 kbyte block
    DataBufferFill(0,1024,0x00,0x11);
    MX_NOR_WriteData(pM2SM, 0x00010000, 16);
    MX_NOR_ReadData(pM2SM, 0x00010000, Mem_page, 16);
    
    //-MX_NOR_WriteCR(pM2SM, 0x07);
    MX_NOR_ReadInfo(pM2SM);
    
    // going back to 1 bit SDR mode
    MX_NOR_WriteCR2(pM2SM, 0,0x00); // switch to 8 bit SDR mode
    MX_NOR_SetMode(pM2SM, 0);
    MX_NOR_ReadInfo(pM2SM);    
    break;
  case 3: // test going to 8 bit SDR then DDR mode
    // DQS is always enabled in DTR mode
    MX_NOR_WriteCR2(pM2SM, 0,0x02); // activate 8 bit DTR mode and reconfigure the HW cell for it
    MX_NOR_SetMode(pM2SM, 2); // 8 bit DTR mode with DQS enabled
