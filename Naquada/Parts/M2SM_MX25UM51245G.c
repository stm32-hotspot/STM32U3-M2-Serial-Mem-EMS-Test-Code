...
// MX25UM51245G 1,8V 1/8 bit NOR Macronix
...
uint8_t JEDECID_MX25UM51245G_1[]={0xC2, 0x81, 0x39, 0x00 };
uint8_t JEDECID_MX25UM51245G_2[]={0xC2, 0x80, 0x3A, 0x00 };
uint8_t JEDECID_MX25UM51245G_3[]={0xFF, 0xFF, 0xFF, 0xFF };

uint8_t *JEDECID_MX25UM51245G[NUMBEROFID]={JEDECID_MX25UM51245G_1, JEDECID_MX25UM51245G_2, JEDECID_MX25UM51245G_3}; 

SerialMemGlobalInfo_t MX_18_SerialMemGlobalInfo =
{
  ...
  .Delay_after_power_up_us = 1500,// tVSL to fill
  ...
  .PageWriteTimeMaxMs = 1,// 0.75ms to fill
...
static const uint32_t CR2_Adresses[] = { 
  0x00000000, // 0
  0x00000200, // 1
  0x00000300, // 2
  0x00000400, // 3
...
uint8_t MX_18_NOR_ReadStatusRegister(M2SerMem_t * pM2SM) {
  ...
  pM2SM->Registers[MX25UM_STATUS_REG_INDEX] = Bytes[0];
...
void MX_18_NOR_WriteEnable(M2SerMem_t * pM2SM) {
  ...
  MX_18_NOR_ReadStatusRegister(pM2SM);// bonus for debug
}
...
void MX_18_NOR_ReadCR(M2SerMem_t * pM2SM) {
  ...
  pM2SM->Registers[MX25UM_CONTROL_REG_INDEX] = Bytes[0];
}
...
void MX_18_NOR_WriteCR(M2SerMem_t * pM2SM, uint8_t Byte) {
  ...
  if(mode>=countof(STs)) while(1);
  SerialTransaction_t ST = STs[mode];
  MX_18_NOR_ReadStatusRegister(pM2SM);
  MX_18_NOR_WriteEnable(pM2SM);  
  
  switch(pM2SM->MemoryConfigurationMode) {
    
  case 0: // 1bit STR
      Bytes[0] = pM2SM->Registers[MX25UM_STATUS_REG_INDEX]; // existing SR value
...
void MX_18_NOR_ReadCR2(M2SerMem_t * pM2SM, uint8_t index) {
  ...
  pM2SM->Registers[MX25UM_CR2_0 + index] = Bytes[0];
}


void MX_18_NOR_WriteCR2(M2SerMem_t * pM2SM, uint8_t index, uint8_t byteval) {
...
void MX_18_NOR_ReadSecurityRegister(M2SerMem_t * pM2SM) {
  ...
  pM2SM->Registers[MX25UM_SECURITY_REG_INDEX] = Bytes[0];
}

void MX_18_NOR_ReadAllRegisters(M2SerMem_t * pM2SM) {
  
  MX_18_NOR_ReadStatusRegister(pM2SM);
  MX_18_NOR_ReadCR(pM2SM);
  MX_18_NOR_ReadAllCR2(pM2SM);
  MX_18_NOR_ReadSecurityRegister(pM2SM);
}
...
//  SerialConfiguration_t t;
  ...
  case 0: // 1bit STR
      MX_18_NOR_ReadCR(pM2SM);
      MX_18_NOR_ReadJedec(pM2SM, JedecInfo);
      MX_18_NOR_ReadSFDP(pM2SM, SFDP);
      MX_18_NOR_ReadData(pM2SM, 0x00000000, DataSegment, 4096);
      MX_18_NOR_ReadAllCR2(pM2SM);
      MX_18_NOR_ReadSecurityRegister(pM2SM);
    break;
...
int8_t MX_18_NOR_GetImpedance(M2SerMem_t * pM2SM) { // to be tested

  MX_18_NOR_ReadCR(pM2SM);
  return pM2SM->Registers[MX25UM_CONTROL_REG_INDEX] & 0x07; // hard coded for now
}

void MX_18_NOR_SetImpedance(M2SerMem_t * pM2SM, int8_t imp) { // to be tested
  
  uint8_t Bytes[2];
  MX_18_NOR_ReadCR(pM2SM);
  Bytes[0] = pM2SM->Registers[MX25UM_CONTROL_REG_INDEX] & ~0x07;
  Bytes[0] |= imp & 0x07;
  MX_18_NOR_WriteCR(pM2SM, Bytes[0]);
}

int32_t MX_18_NOR_GetDummyCycles(M2SerMem_t * pM2SM) { // untested
  
//  uint8_t Bytes[2];
  MX_18_NOR_ReadCR2(pM2SM, 5);
  return pM2SM->Registers[MX25UM_CR2_300] & ~0x07;
}

void MX_18_NOR_SetDummyCycles(M2SerMem_t * pM2SM, int32_t dummycycles) { // untested
  
  uint8_t Bytes[2];
  MX_18_NOR_ReadCR2(pM2SM, 5);
  Bytes[0] = pM2SM->Registers[MX25UM_CONTROL_REG_INDEX] & ~0x07;
  Bytes[0] |= dummycycles & 0x07;
  MX_18_NOR_WriteCR2(pM2SM, 5, Bytes[0]);
}
