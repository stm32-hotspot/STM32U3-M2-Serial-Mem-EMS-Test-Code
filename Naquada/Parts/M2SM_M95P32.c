/* Copyright (c) 2025 STMicroelectronics.
 * All rights reserved.
 *
 * This software is licensed under terms that can be found in the LICENSE file
 * in the root directory of this software component.
 * If no LICENSE file comes with this software, it is provided AS-IS.
 */

#include "main.h"
#include "SerialMem.h"
#include "M2SM_M95P32.h"

extern XSPI_HandleTypeDef hxspi1;

uint8_t JEDECID_M95P32_1[]={0x20, 0x00, 0x16, 0x00 };
uint8_t JEDECID_M95P32_2[]={0xFF, 0xFF, 0xFF, 0xFF };
uint8_t JEDECID_M95P32_3[]={0xFF, 0xFF, 0xFF, 0xFF };

uint8_t *JEDECID_M95P32[NUMBEROFID]={JEDECID_M95P32_1, JEDECID_M95P32_2, JEDECID_M95P32_3};  

SerialMemGlobalInfo_t ST_SerialMemGlobalInfo =
{
  // Names, part number, manufacturer. We support reset, fast SDR, fast DDR, impedence tune
  .CPN = "M95P32",
  .ManufacturerName = "STM",
  .MemoryType = EEP_Type, // SFDP, Flash, Sector erase, Page write model
  .DataPinIndexStart = 0,
  .DataPinIndexEnd = 3,
  .JEDEC_ID = JEDECID_M95P32, // or 0x70 0x18
  .Support_Features = {         
                      .SFDP_Supported = 1,
                      .JEDEC_ID_Supported = 1, 
                      .SDR_1bit_Supported = 1,
                      .SDR_4bit_Supported = 1,
                      .WAITSTATES_Tuning_Supported = 0,
                      .IMPEDANCE_Tuning_Supported = 1,
                      },
//  .WaitStateVsMHzVsXDR = { {6, 66, 66}, {8, 84, 84}, {10, 104, 104}, {12, 104, 104}, {14, 133, 133}, {16, 133, 133}, {18, 133, 133}, {20, 133, 133} },
  
  .MaxFreq_MHz = 80,       // 
  .Vmin_V_x100 = 160,
  .Vmax_V_x100 = 360,
  
  .Delay_after_power_up_us = 30,// tVSL to fill
  
  // 128Mbit
  .SizekByte = 8192, // -1 not available
  .BlockSizeByte = 1024*64L,   // -1 not available
  .BlockEraseTimeMaxMs = 8,// to fill
  .SectorSizeByte = 4096,  // -1 not available
  .SectorEraseTimeMaxMs = 5,// to fill
  .PageSizeByte = 512,    // -1 not available
  .PageWriteTimeMaxMs = 2,// 1.5ms max to fill
  .ChosenErasableAreaByteSize = 4096, // sector size chosen here
  .WriteRegisterTimeMaxMs = 9,// to fill
  .ResetTimeMaxMs = 25,// tRST3? to fill
  //.ResetTimeMaxMs = 1,
  .AutopollingIntervalTime = 0,// to fill // what unit ?
  .RecommendedMinDataWriteChunkCRCSize = 16,
  .ErasedByteValue = 0xFF,
  
/*
  .PresetValues32b = {  0x00000000, // 0
                        0x00000001, // 1
                        0x00000200, // 2
                        0x00000300, // 3
                        0x00000400, // 4
                        0x00000500, // 5
                        0x00000800, // 6
                        0x00000C00, // 7
                        0x00000D00, // 8
                        0x00000E00, // 9
                        0x00000F00, // 10
                        0x40000000, // 11
                        0x80000000, // 12
                        0, // 13
                        0, // 14
  },
*/  
  // put here wait states table
  .SR_WEL_MaskVal = 0x02, // set 1 to activate latch
  .SR_WIP_MaskVal = 0x01, // set 1 when memory is busy
  .SR_RegIndex = 0,// to fill // should be 0
  .SDR1BIT_MaskVal = 0,
  .SDR1BIT_RegIndex = 0,// to fill
  .SRDWideBIT_MaskVal = 0,
  .DDR_MaskVal = 0,
  .DDR_MaskVal_RegIndex = 0,// to fill
  .IMPEDANCE_MaskVal = 0x30,
  .IMPEDANCE_RegIndex = 1,// configuration register  to fill
  .DummyCycleVsMHz = {0},// to fill
  .DummyCycle_MaskVal = 0,// to fill
  .DummyCycle_RegIndex = 0,// to fill
  
} ;

extern XSPI_RegularCmdTypeDef     gCommand;

extern uint8_t JedecInfo[4];
static uint8_t ManufDeviceID[2];
static uint8_t UniqueID[8];
static uint8_t SFDP[256];

static uint8_t Mem_SecurityReg[2];
static uint8_t Mem_SR[3]; // 3 status registers

extern SerialTransaction_t ST;

extern M2SerMem_t * pM2SM;

void ST_NOR_SetMode(M2SerMem_t * pM2SM, uint8_t newmode) {
  
  switch(newmode) {
    
  case 0: // 1 bit SDR
    pM2SM->MemoryBusMode[0] = pM2SM->MemoryBusMode[1] = pM2SM->MemoryBusMode [3] = _S1;
    pM2SM->MemoryBusMode[2] = _NONE;
    pM2SM->MemoryConfigurationMode = 0;
    pM2SM->DQS_Enabled = 0;
    break;
  case 1: // Perf mode (no mode 2). 4 bit address and data when possible
    pM2SM->MemoryBusMode[0] = _S1;
    pM2SM->MemoryBusMode[1] = _S1;
    pM2SM->MemoryBusMode[2] = _NONE;
    pM2SM->MemoryBusMode[3] = _S4;
    
    pM2SM->MemoryConfigurationMode = 1;
    pM2SM->DQS_Enabled = 0;
    break;

  default:
    while(1);
  }
  
}

extern XSPI_HandleTypeDef hxspi1;

uint8_t ST_NOR_ReadStatusRegister(M2SerMem_t * pM2SM) {

  uint8_t Bytes[2];
  
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x05, 0x00000000, 0, 1, 0 }, // 1bit STR
    { 0x05, 0x00000000, 0, 1, 0 }, // 2bit STR?
    { 0x05, 0x00000000, 0, 1, 0 }, // 4bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs))   while(1);
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST); // READ SR
  SM_DoTransactionRead(pM2SM, Bytes);    

  pM2SM->Registers[M95P_STATUS_REG_INDEX] = Bytes[0];
  
  return Bytes[0];
}



uint8_t ST_NOR_ReadConfigAndSafetyRegister(M2SerMem_t * pM2SM) {

  uint8_t Bytes[2];
  
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x15, 0x00000000, 0, 2, 0 }, // 1bit STR
    { 0x15, 0x00000000, 0, 2, 0 }, // 2bit STR
    { 0x15, 0x00000000, 0, 2, 0 }, // 4bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs))   while(1);
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST); // READ SR
  SM_DoTransactionRead(pM2SM, Bytes);    

  pM2SM->Registers[M95P_CONFIG_REG_INDEX] = Bytes[0];
  pM2SM->Registers[M95P_SAFETY_REG_INDEX] = Bytes[0];
  
  return Bytes[0];
}


uint8_t ST_NOR_ReadVolatileRegister(M2SerMem_t * pM2SM) {

  uint8_t Bytes[2];
  
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x85, 0x00000000, 0, 1, 0 }, // 1bit STR
    { 0x85, 0x00000000, 0, 1, 0 }, // 4-line STR ?
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs))   while(1);
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST); // READ SR
  SM_DoTransactionRead(pM2SM, Bytes);    

  pM2SM->Registers[M95P_VOLATILE_REG_INDEX] = Bytes[0];
  
  return Bytes[0];
}


void ST_NOR_WriteStatusRegister(M2SerMem_t * pM2SM, uint8_t Byte) {

  uint8_t Bytes[2];
  
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x01, 0x00000000, 0, 1, 0 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs))   while(1);
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST); // READ SR
  SM_DoTransactionWrite(pM2SM, Bytes);    

//  pM2SM->Registers[M95P_VOLATILE_REG_INDEX] = Byte;
}


void ST_NOR_WriteConfigRegister(M2SerMem_t * pM2SM, uint8_t Byte) {

  uint8_t Bytes[2];
  Bytes[0] = ST_NOR_ReadStatusRegister(pM2SM);
  Bytes[1] = Byte;
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x01, 0x00000000, 0, 2, 0 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs))   while(1);
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST); // READ SR
  SM_DoTransactionWrite(pM2SM, Bytes);    

}


void ST_NOR_WriteVolatileRegister(M2SerMem_t * pM2SM, uint8_t Byte) {

  uint8_t Bytes[2];
  
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x81, 0x00000000, 0, 1, 0 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs))   while(1);
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST); // READ SR
  SM_DoTransactionWrite(pM2SM, Bytes);    

//  pM2SM->Registers[M95P_VOLATILE_REG_INDEX] = Byte;
}


void ST_NOR_WriteEnable(M2SerMem_t * pM2SM) {

  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x06, 0x00000000, 0, 0, 0 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs))   while(1);
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST); // READ SR
  SM_DoTransactionCMD(pM2SM);    
  
  ST_NOR_ReadStatusRegister(pM2SM);// bonus for debug
}

void ST_NOR_WriteDisable(M2SerMem_t * pM2SM) {

  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x04, 0x00000000, 0, 0, 0 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs))   while(1);
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST); // READ SR
  SM_DoTransactionCMD(pM2SM);    
  
  ST_NOR_ReadStatusRegister(pM2SM);// bonus for debug
}


void ST_NOR_ClearSafetyRegister(M2SerMem_t * pM2SM) {

  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x50, 0x00000000, 0, 0, 0 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs))   while(1);
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST); // READ SR
  SM_DoTransactionCMD(pM2SM);    

}


void ST_NOR_WaitBusyEnds(M2SerMem_t * pM2SM) {

  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x05, 0x00000000, 0, 1, 0 }, // 1bit STR
    { 0x05, 0x00000000, 0, 1, 0 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs)) while(1);
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST); // READ SR
  SM_DoTransactionPoll(pM2SM);    
  
  ST_NOR_ReadStatusRegister(pM2SM);// bonus for debug
}


void ST_NOR_ReadData(M2SerMem_t * pM2SM, uint32_t Addr, uint8_t* pBytes, uint32_t ByteSize) {

  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x0B, 0x00000000, 3, 0, 8 }, // 1bit STR
    { 0x0B, 0x00000000, 3, 0, 8 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs)) while(1);
  SerialTransaction_t ST = STs[mode];

  ST.Addr = Addr;
  ST.DataSizeByte = ByteSize;

  SM_PresetCommandParameters(pM2SM, &ST);
  SM_DoTransactionRead(pM2SM, pBytes);
}

void ST_NOR_ReadDataQuad(M2SerMem_t * pM2SM, uint32_t Addr, uint8_t* pBytes, uint32_t ByteSize) {

  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x6B, 0x00000000, 3, 0, 8 }, // 1bit STR
    { 0x6B, 0x00000000, 3, 0, 8 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs)) while(1);
  SerialTransaction_t ST = STs[mode];

  ST.Addr = Addr;
  ST.DataSizeByte = ByteSize;

  
  ST_NOR_SetMode(pM2SM,1);
  SM_PresetCommandParameters(pM2SM, &ST);
  SM_DoTransactionRead(pM2SM, pBytes);
  ST_NOR_SetMode(pM2SM,0);
  
}

void ST_NOR_ReadAllRegisters(M2SerMem_t * pM2SM) {
  
  ST_NOR_ReadStatusRegister(pM2SM);
  ST_NOR_ReadConfigAndSafetyRegister(pM2SM);
  ST_NOR_ReadVolatileRegister(pM2SM);
}


void ST_NOR_ReadJedec(M2SerMem_t * pM2SM, uint8_t* Bytes) {

  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x9F, 0x00000000, 0, 3, 0 }, // 1bit STR
    { 0x9F, 0x00000000, 0, 3, 0 }, // 1bit STR
  };
  
   int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs)) while(1);
  
  SerialTransaction_t ST = STs[mode];

  SM_PresetCommandParameters(pM2SM, &ST);
  SM_DoTransactionRead(pM2SM, Bytes); 
}


void ST_NOR_ReadSFDP(M2SerMem_t * pM2SM, uint8_t* Bytes) {

  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x5A, 0x00000000, 4, 256, 0 }, // 1bit STR
    { 0x5A, 0x00000000, 4, 256, 0 }, // 1bit STR
  };
  
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs)) while(1);
  
  SerialTransaction_t ST = STs[mode];
  
  SM_PresetCommandParameters(pM2SM, &ST);
  SM_DoTransactionRead(pM2SM, Bytes); 
}

void ST_NOR_ReadID(M2SerMem_t * pM2SM, uint8_t* Bytes) {
  
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x8B, 0x00000000, 3, 8, 0 }, // 1bit STR
    { 0x8B, 0x00000000, 3, 8, 0 }, // 4bit STR
  };
  
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs)) while(1);
  
  SerialTransaction_t ST = STs[mode];
  
  SM_PresetCommandParameters(pM2SM, &ST);
  SM_DoTransactionRead(pM2SM, Bytes); 
  
}


void ST_NOR_ReadInfo(M2SerMem_t * pM2SM) {

  ST_NOR_ReadAllRegisters(pM2SM);

  ST_NOR_ReadID(pM2SM, UniqueID);
  
  ST_NOR_ReadJedec(pM2SM, JedecInfo);
  ST_NOR_ReadSFDP(pM2SM, SFDP);
  
  ST_NOR_ReadData(pM2SM, 0x00000000, DataSegment, 4096);
  
  //while(1)
  ST_NOR_ReadDataQuad(pM2SM, 0x00000000, DataSegment, 4096);
}


//////////////////////
void ST_NOR_EraseSegment(M2SerMem_t * pM2SM, uint32_t Addr) {

  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x20, 0x00000000, 3, 0, 0 }, // 1bit STR
    { 0x20, 0x00000000, 3, 0, 0 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs)) while(1);
  SerialTransaction_t ST = STs[mode];
  ST.Addr = Addr;

    ST_NOR_WriteEnable(pM2SM);
    SM_PresetCommandParameters(pM2SM, &ST);
    SM_DoTransactionCMD(pM2SM);
    ST_NOR_ReadStatusRegister(pM2SM);
    ST_NOR_ReadStatusRegister(pM2SM);
    
    ST_NOR_WaitBusyEnds(pM2SM);
    ST_NOR_ReadStatusRegister(pM2SM);
  
  NOPs(1);  
  
}


void ST_NOR_ProgramData(M2SerMem_t * pM2SM, uint32_t Addr, uint32_t SizeByte) {
  
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x0A, 0x00000000, 3, 0, 0 }, // 1bit STR
    { 0x0A, 0x00000000, 3, 0, 0 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs)) while(1);
  SerialTransaction_t ST = STs[mode];
  ST.Addr = Addr;
  ST.DataSizeByte = SizeByte;

    ST_NOR_WriteEnable(pM2SM);
    SM_PresetCommandParameters(pM2SM, &ST);
    SM_DoTransactionWrite(pM2SM, DataSegment);
    ST_NOR_WaitBusyEnds(pM2SM);
    ST_NOR_ReadStatusRegister(pM2SM);
  
  // use security register to avoid reading back all data to confirm proper writing. speeds things up.}
}


void ST_NOR_WriteData(M2SerMem_t * pM2SM, uint32_t Addr, uint32_t SizeByte) {
  
  const SerialTransaction_t STs[3] = {
    // CMD; Addr; AddrSizeByte; DataSizeByte; DummyCycles;
    { 0x0A, 0x00000000, 3, 0, 0 }, // 1bit STR
    { 0x0A, 0x00000000, 3, 0, 0 }, // 1bit STR
  };
  int8_t mode = pM2SM->MemoryConfigurationMode;
  if(mode>=countof(STs)) while(1);
  SerialTransaction_t ST = STs[mode];
  ST.Addr = Addr;
  ST.DataSizeByte = SizeByte;

    ST_NOR_WriteEnable(pM2SM);
    SM_PresetCommandParameters(pM2SM, &ST);
    SM_DoTransactionWrite(pM2SM, DataSegment);
    ST_NOR_WaitBusyEnds(pM2SM);
    ST_NOR_ReadStatusRegister(pM2SM);
  
  // use security register to avoid reading back all data to confirm proper writing. speeds things up.}
}


//========================================================


static uint8_t Impedance = 6;
static volatile uint8_t test_branch = 1;
static uint8_t Mem_page[32];
void M95P32_Test(void)
{
  // prepare autopoll
  SM_ConfigInit(pM2SM);
  ST_NOR_SetMode(pM2SM, 0);  
  
  SerialMemoryTuneDelay();
  //Run the first test in 1 bit mode
  //HAL_XSPI_SetClockPrescaler(&hxspi1, 4);
  //ST_NOR_ReadInfo(pM2SM);
  //ST_NOR_WriteCR(Impedance);
//  HAL_XSPI_SetClockPrescaler(&hxspi1, 1);
  //while(1)
    ST_NOR_ReadInfo(pM2SM);

  switch(test_branch) {
  case 1:
    ST_NOR_ReadStatusRegister(pM2SM);
    ST_NOR_WriteEnable(pM2SM);
    ST_NOR_ReadStatusRegister(pM2SM);
    ST_NOR_WriteDisable(pM2SM);
    ST_NOR_ReadStatusRegister(pM2SM);
    
    //while(1); // stop here for now
    
    ST_NOR_EraseSegment(pM2SM, 0);
    
    DataBufferFill(0,512,0x55,1);
    ST_NOR_ReadInfo(pM2SM);
    DataBufferFill(0,1024,0,1);
    ST_NOR_ProgramData(pM2SM, 0x00000000, 256);
    DataBufferFill(0,1024,0,0);
    ST_NOR_ReadInfo(pM2SM);
    DataBufferFill(0,1024,0x80,0xFF);
    ST_NOR_ProgramData(pM2SM, 0x00000100, 16);
    ST_NOR_ReadInfo(pM2SM);
    break;
  case 2: // test going to 8 bit SDR mode then back to 1 bit SDR mode
    // going to 8 bit SDR mode
    ST_NOR_ReadInfo(pM2SM);
    ST_NOR_ReadInfo(pM2SM);
    ST_NOR_ReadInfo(pM2SM);    
    // program page in 4 bit mode
    break;
  case 4:
    NOPs(1);
    break;
  default: while(1);
  }
    
  if( (SFDP[0] == 'S') && (SFDP[1] == 'F') && (SFDP[2] == 'D') && (SFDP[3] == 'P') ) {
    // green LED
    BriskDimTimedLED(11,100); /* Green ON */
    BriskDimTimedLED(12,0); /* Red OFF */
  }else
  { // red LED
    BriskDimTimedLED(11,0);
    BriskDimTimedLED(12,100);   
    
  }
  
  //return;
  
  while(1)
    NOPs(1);
  
}












